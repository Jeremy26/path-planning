      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          int prev_size = previous_path_x.size();
            ...
            // Later these will be incorporated with a spline and filled with more points that control speed
        ...
        for (int i=0; i<sensor_fusion.size();i++){
          ...
          check_car_s +=((double)prev_size*.02*check_speed); // Compute car s
        ...
        if(too_close){
          ...
          double x_add_on = 0; //Start at the origin
            ...
            for (int i =1; i<=50 - previous_path_x.size();i++){
              double N = (target_dist/(0.02*ref_vel/2.24)); //Update every .02 seconds, at 2.24 meters per second
              ...
              // Rotate back to normal after rotate earlier
